...
public class LogRecService {
	...
	public void saveMatchLog(ArrayList<MatchedLogRec> matchLogs) {
		try (AppendObjectOutputStream obs = new AppendObjectOutputStream(
	...
	public ArrayList<MatchedLogRec> readMatchLog() {
	   ...
	   //需补充该方法√
		...
		try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(
				"MatchedLogs.txt"));
				FileInputStream fis = new FileInputStream("MatchedLogs.txt");) {
			...
			while (true) {
				// 将对象添加到泛型集合中	
				try {
				if((matchLog = (MatchedLogRec) ois.readObject())!=null)
		...
		//System.out.println("read"+matchLogs.size());
		return matchLogs;
	}
	...
	public ArrayList<MatchedLogRec> readMatchedLogFromDB()
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				MatchedLogRec matchLog = new MatchedLogRec(login,logout);
				matchLogs.add(matchLog);
