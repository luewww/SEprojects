...
public class MainFrametest2 extends JFrame{
	...
	private JMenuItem miGather, miSave, miSend,
			miShow, miExit, miCheck, miAbout,miMLog,miMTrans;
	...
	// 构造方法
	public MainFrametest2(String s) {
		// 调用父类的构造方法
	...
	private void initMenu() {
		...
		miMLog =new JMenuItem("匹配日志数据");
		menuMatch.add(miMLog);
		
		miMTrans =new JMenuItem("匹配物流数据");
		menuMatch.add(miMTrans);
	...
	private void initToolBar() {
		...
		btnGather.setSize(600, 50);
		// 注册监听
		btnGather.addActionListener(new GatherListener());
		toolBar.add(btnGather);

		ImageIcon matchLIcon = new ImageIcon("images\\matchLData.png");
		btnMatchLog = new JButton("匹配日志数据", matchLIcon);
		// 注册监听
		...
		ImageIcon matchTIcon = new ImageIcon("images\\matchTData.png");
		btnMatchTrans = new JButton("匹配物流数据", matchTIcon);
		// 注册监听
	...
	private void initLogGatherGUI() {
			...
			lblLogId = new JLabel("日志ID："+getLogid());
			pLogId.add(lblLogId);
			...
			lblName = new JLabel("用户名："+this.user);
			pName.add(lblName);
			...
			// 添加重置按钮监听
			btnLogReset.addActionListener(new logResetListener());
	...
	private void initTransGatherGUI() {
		...
		lblTransId = new JLabel("物流ID："+getTranid());
		pTransId.add(lblTransId);

		/*txtTransId = new JPanel();
		...
		cmbTranStatus.setSize(new Dimension(20, 20));
		...
		btnTranReset.addActionListener(new tranResetListener());
	...
	private class GatherListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// 切换主面板的卡片为采集面板
			lblLogId.setText("日志ID："+getLogid());
			lblTransId.setText("物流ID："+getTranid());
	...
	private int getLogid()
	{
		...
		dbl=logModel.getRowCount();
		//System.out.print("dbl"+dbl);
		for (LogRec a : logList) {
			if (a != null) 
				listl++;
		}
	...
	private int getTranid()
	{
		...
		MatchedTableModel transModel = new MatchedTableModel(
			...
			dbl=transModel.getRowCount();
		for (Transport a : transList) {
			if (a != null) 
				listl++;
		}
		return dbl+listl+1;
	...
	private class GatherTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 设置物流类型
			int transportType = cmbTranStatus.getSelectedIndex() + 1;
			// 将数据包装成物流对象
	...
	// 重置按钮监听类
	private class tranResetListener implements ActionListener {
		// 重置按钮的事件处理方法
		public void actionPerformed(ActionEvent e) {
	...
	private class logResetListener implements ActionListener {
		// 重置按钮的事件处理方法
		public void actionPerformed(ActionEvent e) {
			//txtName.setText("");
			//txtLogId.setText("");
	...
	private class MatchLogListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 显示对话框
			if(matchedLogs==null)
				JOptionPane.showMessageDialog(null, "无匹配物流数据！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			else
	...
	private class MatchTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(matchedTrans==null)
				JOptionPane.showMessageDialog(null, "匹配物流为空！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			else
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(matchedLogs!=null && matchedLogs.size()>0)
			{
				...
				JOptionPane.showMessageDialog(null, "无匹配日志数据！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			...
			/*for(MatchedLogRec a:matchedLogs) {
				if(logList.contains(a))logList.remove(a);
			}
			for(MatchedTransport a:matchedTrans) {
				if(transList.contains(a))transList.remove(a);
			}*/
			...
			try {
			     ...
			     if (matchedLogs != null && matchedLogs.size() > 0) {
			      //创建Socket发送日志信息，日志信息发送到服务器的6666端口
			    	 Socket logSocket = new Socket(serverIP,6666);//创建用于序列化匹配日志信息对象的输出流
			    	 ObjectOutputStream logOutputStream = new ObjectOutputStream(logSocket.getOutputStream());//向流中写入匹配的日志信息
			      ...
			      //补充代码，创建Socket发送物流信息， 日志信息发送到服务器的6668端口
			    	  //创建Socket发送日志信息，日志信息发送到服务器的6666端口
				      Socket transSocket = new Socket(serverIP, 6668);//创建用于序列化匹配日志信息对象的输出流
				      ObjectOutputStream transOutputStream = new ObjectOutputStream(transSocket.getOutputStream());//向流中写入匹配的日志信息
